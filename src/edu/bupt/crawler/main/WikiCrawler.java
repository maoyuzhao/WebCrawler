...
public class WikiCrawler {
	...
	private int parserType; // 选择parser类型，分别为jsoupParser和boilerpipeParser，默认为jsoupParser
	...
	public void beginCrawl() {
		...
		if (storageType.equalsIgnoreCase("single")) {
			this.storageType = WikiCrawler.SINGLE;
		} else {
			this.storageType = WikiCrawler.MULTIPLE;
		}

		String parserType = config.getString("crawler.parse.parserType");
		if (parserType.toLowerCase().equals("jparser")) {
			this.parserType = WikiCrawler.JParser;
		} else if (parserType.equalsIgnoreCase("bparser")) {
			this.parserType = WikiCrawler.BParser;
		} else if (parserType.equalsIgnoreCase("mixparser")) {
			this.parserType = WikiCrawler.MixParser;
		} else {
			System.err.println("ParserType config error, please check!");
			System.exit(1);
		}
	...
	private void initContainer() throws Exception {
		...
		Collections.sort(urlFileQueue, (f1, f2) -> f1.getName().compareTo(f2.getName()));
	}
	...
	public class Parser implements Runnable {
		 ...
		 * @return
		 */
		public String jsoupParser(String html, List<?> acSelector, List<?> rjSelector) {
			Document doc = Jsoup.parse(html);
		 ...
		 * @throws Exception
		 */
		private boolean parse(String html, Document doc, String url) throws Exception {
			String content = null;
			boolean isParsed = false;
			if (useParse) {
				switch (parserType) {
				case JParser:
					content = jsoupParser(doc, acSelector, rjSelector);
					break;
				case BParser:
				...
				if (content != null) {
					...
					pVisitedCache.add(url);
					System.out.printf("%-16s %-10s\t%s\n", Thread.currentThread().getName(), "\\parse\\", url);
				} else {
					isParsed = false;
					upVisitedCache.add(url);
					System.err.printf("%-16s %-10s\t%s\n", Thread.currentThread().getName(), "/unparse/", url);
				}
			} else {
				upVisitedCache.add(url);
				System.out.printf("%-16s %-10s\t%s\n", Thread.currentThread().getName(), "/fetch/", url);
			}
			return isParsed;
		}
		...
		private void filterUrl(Collection<String> links) {
			if (urlQueue.size() > 500 || !urlFileQueue.isEmpty()) {
				synchronized (bfUrl) {
		...
		private void store(String url, final String html, boolean isParsed) throws Exception {
			switch (storageType) {
			case WikiCrawler.SINGLE:
				if (isStoreParsedHtml || (!isStoreParsedHtml && !isParsed)) {
					storeSingle(storePath, url, html, isParsed);
				}
				break;
			case WikiCrawler.MULTIPLE:
				if (isStoreParsedHtml || (!isStoreParsedHtml && !isParsed)) {
					storeMultiple(storePath, url, html, isParsed);
				}
				break;
			default:
				break;
		...
		public void run() {
			...
			while (true) {
				try {
					...
					doc = pairs.getValue();
					html = doc.html() + "\n";
					Collection<String> links = extract(doc, acRegex, rjRegex);
					filterUrl(links);
					if (useDateFilter && !JudgeContent.isDateLegal(html, startDate, endDate, dateRegex)) {
						FileUtils.writeStringToFile(rejectedUrlFile, url, "utf-8", true);
						continue;
					}
					isParsed = parse(html, doc, url);
					store(url, html, isParsed);
				} catch (Exception e) {
					logger.warn(e.getMessage(), e);
				}
			}
	...
	private class Loger implements Runnable {
		...
		private void urlQueueLog() throws Exception {
			...
			} else if (urlQueueSize < 1000 && urlFileQueueSize > 0) {
